...
public sealed class CreateMentorCommandHandler : IRequestHandler<CreateMentorCommand, CreateMentorResponse>
{
    ...
    public async Task<CreateMentorResponse> Handle(CreateMentorCommand request, CancellationToken cancellationToken)
    {
        ...
        // Create mentor.
        await _mentorRepository.CreateAsync(user.Id, request.TechnologyId, DateTimeOffset.UtcNow, cancellationToken);
        ...
        await _emailService.SendMentorInvitationAsync(user.Username, user.Email, invitationToken);

        return new CreateMentorResponse
        {
            Success = true,
            Message = "Mentor created successfully and the initial setup email was sent.",
            InvitationLink = null
        };
    }
}