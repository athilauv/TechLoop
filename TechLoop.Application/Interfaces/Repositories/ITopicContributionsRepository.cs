...
public interface ITopicContributionRepository
{
    ...
    Task<bool> ReviewAsync(int contributionId, short status, string? reviewNotes, int? position, int? parentSubTopicId, Guid reviewedBy, CancellationToken cancellationToken);
    ...
    Task<IEnumerable<TopicContributionPendingResponse>> GetPendingContributionsAsync(Guid mentorId, CancellationToken cancellationToken);
    Task<TopicContributionResponse?> GetMentorContributionByIdAsync(Guid mentorId, int contributionId, CancellationToken cancellationToken);
}