...
function validateQuestion(
    ...
    return first(
        ...
        requireText(
            data.title,
            update ? "Title cannot be empty" : "Title is required.",
        ),
        !max(data.title, 200)
        ...
        requireText(
            data.slug,
            update ? "Slug cannot be empty" : "Slug is required.",
        ),
        !max(data.slug, 200)
...
function validateCodingTemplate(
    data: AnyRecord,
    update = false,
): string | null {
    return first(
...
function validateTestCase(
    ...
    return first(
        ...
        !update && data.input == null
            ? "Input is required."
            : null,
        !update && data.expectedOutput == null
            ? "Expected output is required."
            : null,
        update && isEmpty(data.input)
            ? "Input is required."
            : null,
        update && isEmpty(data.expectedOutput)
            ? "Expected output is required."
            : null,
        !isPositive(data.position)
...
function validateComment(
    ...
    return first(
        !update
            ? requirePositive(
                data.postId ?? data.discussionId,
                "Post/Discussion id is required.",
            )
            : null,
        update && !isPositive(data.id)
            ? "Invalid comment."
        ...
        !isPositive(data.parentCommentId)
            ? "ParentCommentId must be greater than 0."
            : null,
    );
}

function validateCommunityPost(
    data: AnyRecord,
): string | null {
    return first(
        requireText(data.title, "Title is required."),
        !max(data.title, 200)
            ...
            : null,
        requireText(data.content, "Content is required."),
        data.technologyId != null &&
        !isPositive(data.technologyId)
...
function validateProfile(
    ...
    return first(
        ...
        (
            ...
            !isUrl(text(data.linkedInUrl))
        )
            ? "Invalid LinkedIn URL."
            : null,
        !max(data.githubUrl, 500) ||
        (
            !isEmpty(data.githubUrl) &&
            !isUrl(text(data.githubUrl))
        )
            ? "Invalid GitHub URL."
            : null,
...
function validateContributionReview(
    data: AnyRecord,
): string | null {
    ...
    return first(
        ...
        !isPositive(data.position)
            ...
            : null,
        status === 2 &&
        data.parentSubTopicId != null &&
        !isPositive(data.parentSubTopicId)
...
function validateSubmissionResult(
    data: AnyRecord,
): string | null {
    const values = [
...
function validateRequest(
    ...
    if (
        url.startsWith("admin/technologies") &&
        m === "POST"
    ) {
    ...
    if (
        url === "admin/technology-categories" &&
        m === "POST"
    ) {
    ...
    if (
        url.startsWith("admin/technology-categories/") &&
        m === "PUT"
    ) {
    ...
    if (
        url.match(/^mentor\/topics\/\d+$/) &&
        m === "PUT"
    ) {
        return validateTopic(data);
    }

    if (url.startsWith("mentor/subtopics") &&
        m === "POST"
    ) {
    ...
    if (
        url.match(
            /^mentor\/questions\/\d+\/mcq_options$/,
        ) &&
        m === "POST"
    ) {
        return validateMcqOption({
            ...data,
            questionId: idInPath,
        });
    }
    ...
    if (
        url.match(/^mentor\/mcq-options\/\d+$/) &&
        m === "PUT"
    ) {
    ...
    if (
        url.match(
            /^mentor\/questions\/\d+\/coding-templates$/,
        ) &&
        m === "POST"
    ) {
        return validateCodingTemplate({
            ...data,
            questionId: idInPath,
        });
    }
    ...
    if (
        url.startsWith("mentor/questions") &&
        m === "POST"
    ) {
    ...
    if (
        url.match(/^mentor\/posts\/\d+\/comments$/) &&
        m === "POST"
    ) {
    ...
    if (
        url.match(
            /^mentor\/discussions\/\d+\/(?:pin|unpin)$/,
        ) &&
        m === "PATCH"
    ) {
        return requirePositive(
            idInPath,
            "Discussion id must be greater than 0.",
        );
    }

    if (
        url.match(/^mentor\/discussions\/\d+$/) &&
        m === "DELETE"
    ) {
        return requirePositive(
            idInPath,
    ...
    if (
        url.startsWith("api/discussions") &&
        m === "POST" &&
    ...
    if (
        url.startsWith("api/submissions/mcq") &&
        m === "POST"
    ) {
        return first(
            requirePositive(
    ...
    if (
        url.startsWith("api/topic-contributions") &&
        m === "POST"
    ) {
        return first(
            requirePositive(
            ...
            !max(data.referenceUrl, 500)
                ? "ReferenceUrl must not exceed 500 characters."
                : null,
            !max(data.example, 5000)
                ? "Example must not exceed 5000 characters."
                : null,
        );
    }
    ...
    if (
        url.match(
            /^mentor\/topic-contributions\/\d+\/review$/,
        ) &&
        m === "PUT"
    ) {
        return validateContributionReview({
            ...data,
            id: data.id ?? idInPath,
        });
    }
...
export const assertBackendValidation = (
    ...
    const message = getBackendValidationMessage(
        method,
        url,
        data,
    );
