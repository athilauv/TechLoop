...
const validatePassword = (value: unknown, field: string): string | null => {
...
function validateCategory(data: AnyRecord, update = false): string | null {
    ...
    return first(
        requireText(data.name, "Name is required."),
        !max(data.name, limit)
            ? `Name must not exceed ${limit} characters.` : null,
    );
}
...
function validateTestCase(data: AnyRecord, update = false): string | null {
    return first(update && !isPositive(data.id)
        ...
        !isPositive(data.position) ? "Position must be greater than zero." : null,
    );
}
...
function validateSubmission(data: AnyRecord): string | null {
    return first(data.userId !== undefined &&
        isEmpty(data.userId) ? "UserId is required." : null,
        ...
        !max(data.sourceCode, 100000) ? "Source code cannot exceed 100000 characters." : null,
    );
}
...
function validateAuth(path: string, data: AnyRecord): string | null {
    ...
    if (path.endsWith("/register")) {
        return first(
            ...
            text(data.username).trim().length < 3 ? "Username must be at least 3 characters." : null,
            requireText(data.email, "Email is required."),
            text(data.email) !== text(data.email).trim() ? "Email cannot contain leading or trailing spaces." : null,
    ...
    if (path.endsWith("/forgot-password")) {
        return first(
            ...
            text(data.email) !== text(data.email).trim() ? "Email cannot contain leading or trailing spaces." : null,
            !isEmpty(data.email) &&
            !isMentorEmail(text(data.email)) ? "Invalid email format." : null,
        );
    }
    ...
    if (path.endsWith("/mentor-setup")) {
        return first(
            validatePassword(data.password, "Password"),
            requireText(data.confirmPassword, "ConfirmPassword is required.",),
            data.confirmPassword !== data.password ? "Password and Confirm Password do not match." : null,
...
function validateSubmissionResult(data: AnyRecord): string | null {
    ...
    return first(!isPositive(data.id)
            ? "Id must be greater than 0." : null,
        ...values.map(([key, label]) =>
            data[key] != null && !isNonNegative(data[key]) ? `${label} must be greater than or equal to 0` : null,
        ),
    );
}
...
function validateRequest(method: string, rawUrl: string, data: AnyRecord): string | null {
    ...
    if (url.startsWith("auth/")) {
        return validateAuth(url, data);
    }

    if (url.startsWith("api/judge0/run")) {
        return first(
            requirePositive(data.questionId, "A valid question is required."),
            requireText(data.sourceCode, "Source code is required."),
        );
    }
    ...
    if (url.startsWith("api/judge0/submit")) {
        ...
        return first(
            request == null ? "Request is required." : null,
            requireText(request?.sourceCode, "Source code is required.",),
            requirePositive(request?.languageId, "A valid language is required.",),
        );
    }
    ...
    if (url.startsWith("admin/technologies") && m === "POST") {
        return validateTechnology(data);
    }

    if (url.startsWith("admin/technologies/") && m === "PUT") {
        return validateTechnology(data);
    }

    if (url === "admin/technology-categories" && m === "POST") {
        return validateCategory(data);
    }

    if (url.startsWith("admin/technology-categories/") && m === "PUT") {
        return validateCategory(data, true);
    }

    if (url.startsWith("mentor/profile") && m === "PUT") {
        return validateProfile(data);
    }

    if (url.startsWith("mentor/topics") && m === "POST") {
        return validateTopic(data);
    }

    if (url.match(/^mentor\/topics\/\d+$/) && m === "PUT") {
        return validateTopic(data);
    }

    if (url.startsWith("mentor/subtopics") && m === "POST") {
        return validateSubTopic(data);
    }

    if (url.match(/^mentor\/subtopics\/\d+$/) && m === "PUT") {
        return validateSubTopic(data, true);
    }
    ...
    if (url.startsWith("mentor/questions") && m === "POST") {
    ...
    if (url.match(/^mentor\/posts\/\d+\/comments$/) && m === "POST") {
        return validateComment({...data, postId: idInPath});
    }

    if (url.match(/^mentor\/comments\/\d+$/) && m === "PUT") {
        return validateComment({...data, id: idInPath,}, true,);
    }

    if (url.match(/^mentor\/discussions\/\d+\/(?:pin|unpin)$/,) && m === "PATCH") {
        return requirePositive(idInPath, "Discussion id must be greater than 0.",);
    }

    if (url.match(/^mentor\/discussions\/\d+$/) && m === "DELETE") {
        return requirePositive(idInPath, "Invalid discussion id.");
    }

    if (url.startsWith("api/discussions") && m === "POST" && !url.includes("/comments")) {
        return validateDiscussion(data);
    }

    if (url.match(/^api\/discussions\/\d+$/) && m === "PUT") {
        return validateDiscussion({ ...data, id: idInPath}, true);
    }

    if (url.match(/^api\/discussions\/\d+$/) && m === "DELETE") {
        return requirePositive(idInPath, "Invalid discussion id.",);
    }

    if (url.match(/^api\/discussions\/\d+\/comments$/,) && m === "POST") {
        return validateComment({...data, discussionId: idInPath,});
    }
    ...
    if (url.match(/^api\/discussions\/comments\/\d+$/,) && m === "DELETE") {
        return requirePositive(idInPath, "Invalid comment.",);
    }
    ...
    if (
        url.match(/^mentor\/topic-contributions\/\d+\/review$/,) && m === "PUT") {
...
export const getBackendValidationMessage = (method: string, url: string, data: unknown): string | null => {
...
export const assertBackendValidation = (method: string, url: string, data: unknown): void => {
