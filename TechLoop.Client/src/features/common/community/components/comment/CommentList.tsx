...
export default function CommentList({
    ...
    if (comments.length === 0) {
        return (
            <div className="rounded-xl border border-dashed border-[#1e3254] bg-[#081423] px-5 py-7 text-center">
                <p className="text-xs font-medium text-white">No comments yet.</p>
                <p className="mt-1 text-[10px] text-[#526d8e]">Be the first to join the discussion.</p>
            </div>
        );
    }
    ...
    function renderComments(parentId: number | null, depth: number): React.ReactNode {
    ...
    return <div className="space-y-2">{renderComments(null, 0)}</div>;
}
