...
export function getCurrentUserFromStorage(providedUserId?: string): StoredCurrentUser {
    ...
    if (token && (!id || !username)) {
        try {
            ...
            if (parts.length === 3) {
                ...
                if (!id) {
                    const tokenUserId = payload["sub"] ?? payload["userId"] ?? payload["nameid"] ??
                ...
                if (!username) {
                    const tokenUsername =
                        payload["unique_name"] ?? payload["username"] ??
...
export function isOwnedByCurrentUser(
    ...
    return (
        (!!normalizedCurrentUserId && !!normalizedEntityUserId && normalizedCurrentUserId === normalizedEntityUserId) ||
        (!!normalizedCurrentUsername && !!normalizedEntityUsername && normalizedCurrentUsername === normalizedEntityUsername));
}
